...
async def start_challenge(
    ...
    uid = current_user["id"]
    async with conn.execute(
        """INSERT INTO challenge_attempts(user_id, template_key, title, total_days, started_on)
           VALUES (?, ?, ?, ?, date('now','localtime'))""",
        (uid, template["key"], template["name"], template["total_days"]),
    ) as c:
...
async def checkin(
    ...
    if not template or body.rule_key not in {r["key"] for r in template["rules"]}:
